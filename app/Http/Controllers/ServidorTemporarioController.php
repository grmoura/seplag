...
class ServidorTemporarioController extends Controller
{
    ...
    {
        $servidorTemporarioExiste = ServidorTemporario::where('pes_id', $pes_id)->first();

        if (!$servidorTemporarioExiste)
            return response()->json(['message' => 'Servidor Temporario não encontrado.'], Response::HTTP_OK);

        try {
            DB::beginTransaction();

            ServidorTemporario::where('pes_id', $pes_id)->delete();
            ...
            return response()->json([
                'message' => 'Erro ao deletar Servidor Temporario.',
                'error' => $e->getMessage(),
            ], Response::HTTP_INTERNAL_SERVER_ERROR);
        }
    }
